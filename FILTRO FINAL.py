...
line_Angle_X, = ax1.plot([], [], label="X angle with complementary filter",linestyle=':', linewidth=3)

line_angleYacc, = ax2.plot([], [], label="Y angle with acceleration")
line_angleYgyro, = ax2.plot([], [], label="Y angle with gyroscope")
line_Angle_Y, = ax2.plot([], [], label="Y angle with complementary filter", linestyle=':', linewidth=3) 

line_angleZacc, = ax3.plot([], [], label="Z angle with acceleration")
line_angleZgyro, = ax3.plot([], [], label="Z angle with gyroscope")
line_Angle_Z, = ax3.plot([], [], label="Z angle with complementary filter", linestyle=':', linewidth=3)
...
def Complementary_Filter(angleYacc, gy, angleXacc, gx, angleZacc, gz, dt):
    ...
    Angle_Y =  0.02* (Angle_Y_prev + ((gy/16.4)*dt)) + 0.98 * angleYacc
    Angle_X =  0.02* (Angle_X_prev + ((gx/16.4)*dt)) + 0.98 * angleXacc
    Angle_Z =  0.02* (Angle_Z_prev + ((gz/16.4)*dt)) + 0.98 * angleZacc
    ...
    if ser.in_waiting > 0:
        ...
        if len(data) == 6:  
            try:
                ...
                Angle_Y, Angle_X, Angle_Z = Complementary_Filter(angleYacc, gy, angleXacc, gx, angleZacc, gz, dt)
